...
def consensus_reward_func(
    prompts, completions, weighting=200000.0, logging=False, **kwargs
) -> list[float]:
    # Validate inputs
...
def question_recreation_reward_func(
    prompts, completions, weighting=100000.0, logging=False, **kwargs
) -> list[float]:
    # Validate inputs
    if prompts is None or not prompts or not isinstance(prompts, list):
        return [0.0]
...
def strict_format_reward_func(
    completions, weighting=50000.0, logging=False, **kwargs
) -> list[float]:
...
def xmlcount_reward_func(
    completions, weighting=100000.0, logging=False, **kwargs
) -> list[float]:
    # Validate inputs
    if completions is None or not completions or not isinstance(completions, list):
        return [0.0]
