...
def correctness_reward_func(
    prompts, completions, answer, weighting=200000.0, logging=False, **kwargs
) -> list[float]:
    # Validate inputs
...
def int_reward_func(completions, weighting=50000.0, **kwargs) -> list[float]:
    responses = [completion[0]["content"] for completion in completions]
    extracted_responses = [extract_xml_answer(r) for r in responses]
    return [1.0 * weighting if r.isdigit() else 0.0 for r in extracted_responses]


def strict_format_reward_func(completions, weighting=50000.0, **kwargs) -> list[float]:
...
def soft_format_reward_func(completions, weighting=50000.0, **kwargs) -> list[float]:
    """Reward function that checks if the completion has a specific format."""
    # Validate inputs
    if completions is None or not completions or not isinstance(completions, list):
        return [0.0]
...
def xmlcount_reward_func(completions, weighting=100000.0, **kwargs) -> list[float]:
    # Validate inputs
    if completions is None or not completions or not isinstance(completions, list):
        return [0.0]
